...
<div class="entry-meta eventorganiser-event-meta">
	...
	<?php if( eo_is_all_day() ){
		...
		$date_format = 'l, F d, Y ' . get_option('time_format'); 
	} ?>
	<hr>
	...
	<!-- Event details -->
		...
		<?php if( eo_reoccurs() ):?>
			...
			<?php if($next): ?>
				...
				<?php printf('<p>'.__('This event is running from %1$s until %2$s. It is next occurring on %3$s','eventorganiser').'.</p>', eo_get_schedule_start('l, j F Y'), eo_get_schedule_last('l, j F Y'), $next);?>
	...
	<ul class="eo-event-meta" style="float:left;width:50%">
